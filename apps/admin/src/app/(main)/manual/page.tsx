...
const CHANGELOG: { date: string; summary: string }[] = [
  ...
  { date: '2026-09-14', summary: '売上管理の「月次実績・売上予測」タブにある「事業別の売上」表で、確定実績・確定＋見込みの金額をクリックすると、その事業（補助金事業／WEB事業／その他）に含まれる明細の一覧（日付・顧客・カテゴリ・金額・区分）がポップアップで確認できるようにしました。' },
  { date: '2026-09-14', summary: '実績報告サポートの画面から「＋新規案件」で追加した場合、案件進捗（見込み〜申請中）を経由せず、最初からこの一覧にだけ表示されるようにしました。今まではここで新規登録しても案件進捗側の「見込み」に入ってしまい表示されず、後で採択済みにしても今度は案件進捗の「採択」列にも二重に出てしまう問題がありました。' },
...
const GUIDES: { title: string; badge?: string; steps: string[] }[] = [
  ...
  {
    title: 'HP（ホームページ）のセミナー告知投稿',
    steps: [
      'salud-web.jp（会社HP）の「セミナー」ページに新しい告知を出したいときの手順です。WordPress管理画面の同期パターンに用意した「セミナー告知ひな形（コピーして使ってください）」を使うと、コード編集なしで同じ体裁の告知記事を作れます。',
      '手順：WordPress管理画面 → 投稿 → セミナー → 新規追加 → ブロック挿入メニュー（＋）→「パターン」タブ →「セミナー告知ひな形」を選択して挿入します。',
      '挿入されたひな形の【】で囲まれた部分（開催日時／リード文／背景説明／第1部・第2部のタイトル・講師・ポイント／個別相談の相談内容／補足見出し／申込みURL×2箇所など）を実際の内容に書き換え、記事タイトルを入力して「公開」します。',
      '※ 過去のセミナー投稿は上書きせず、セミナーごとに必ず「新規追加」から新しい投稿を作成してください。このカスタム投稿タイプ（セミナー）は変更履歴が残らないため、既存の投稿を上書きすると元の内容は復元できません。',
      '※ 投稿ごとにURLが自動採番されます。固定ページの「/seminar/」はアーカイブ一覧と衝突して使えないため作成しないでください。管理は投稿一覧（投稿 > セミナー）から行います。',
      '※ 内容が空欄・下書き途中の投稿は「公開」のままにしないでください。セミナー一覧に空の記事として表示されてしまいます。',
    ],
  },
]
