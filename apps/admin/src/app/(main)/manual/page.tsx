...
const CHANGELOG: { date: string; summary: string }[] = [
  { date: '2026-09-01', summary: '案件管理に「全案件／補助金／WEB」の絞り込みを追加しました。ボードとカラーの凡例が選んだ種類に合わせて切り替わります。' },
  { date: '2026-09-01', summary: '売上管理に「成功報酬（採択）入金見込み」の一覧を追加しました。採択が決まった補助金案件の成功報酬の合計と、案件ごとの金額・採択日を確認できます（実際の入金はクライアントへの補助金支給後になるため、資金繰り確認用です）。' },
  { date: '2026-09-01', summary: '請求書・見積書の作成・編集画面で、下までスクロールしなくても保存ボタンが常に見える位置に固定されるようにしました。' },
  { date: '2026-09-01', summary: 'スケジュールに「月表示」を追加しました。ヘッダーの「週表示／月表示」で切り替えられ、月表示ではカレンダー形式で1か月分の予定を一覧できます（各日3件まで表示、超過分は「＋N件」）。' },
  { date: '2026-09-01', summary: '売上管理の「月次実績・売上予測」に、月別の縦棒グラフを追加しました。金額の目盛りと、月割りにした目標ラインを薄い点線で表示するので、実績が目標に対してどのくらいかひと目でわかります。棒にマウスを乗せるとその月の金額を表示します。' },
  { date: '2026-09-01', summary: '売上管理の売上台帳から、顧客管理に登録済みの顧客と紐付けられるようにしました（追加・編集フォームの「顧客管理との紐付け」欄）。紐付けた行は顧客名がリンクになり、クリックでその顧客の詳細ページを開けます。' },
  { date: '2026-09-01', summary: '受信トレイに「表示中を一斉削除」ボタンを追加しました。チャネル・ステータスで絞り込んだ状態のまま、表示されているメッセージをまとめて削除できます（確認ダイアログあり）。' },
  { date: '2026-09-01', summary: '請求書・見積書の詳細・作成・編集画面を、これまでの小さいポップアップから画面いっぱいに近い表示に変更しました。' },
  { date: '2026-09-01', summary: '請求書・見積書の明細行を▲▼ボタンで並び替えできるようにしました。あわせて、印刷・PDF・プレビュー時に入力していない空欄の行が表示される不具合と、「作業内容」を使っていないのに列だけ表示される見た目を修正しました（作業内容は、どの明細にも入力がない場合は列ごと非表示になります）。' },
  { date: '2026-08-28', summary: '役員月報に「年間の振り返り」を追加しました。「年間分析を実行」を押すと、選択中の年（1〜12月分）の役員月報をまとめてAIに読ませ、年間の総括・主な成果・通年の課題・来年に向けた議論テーマ・提言を生成します。月次のAI分析と同様、年に1件保存され、あとから見返せます。' },
  { date: '2026-08-28', summary: '役員月報「AI分析」の結果表示を見やすく整理しました。全体まとめをカードで強調し、好調な点（緑）・共通の課題（黄）・月末会議のアジェンダ（青）・アドバイス（紫）を色分けしました。' },
  { date: '2026-08-28', summary: '役員月報「AI分析」の結果をその月に1件保存し、あとから同じ月を開いたときに再生成せず見返せるようにしました（「再分析する」で上書き再生成できます）。あわせて、実際の月報内容で分析すると応答が長くなり生成に失敗する不具合を修正しました。' },
  { date: '2026-08-28', summary: '役員月報に「AI分析」ボタンを追加しました。クリックすると、その月の役員全員分の月報をAIがまとめて読み、全体の要約・好調な点・共通の課題・月末会議のアジェンダ・アドバイスを生成します。役員のみ利用できます。' },
...
const GUIDES: { title: string; badge?: string; steps: string[] }[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      'ヘッダーの「全案件／補助金／WEB」で、ボードに表示する案件の種類を絞り込めます。',
  ...
  },
  {
    ...
    steps: [
      ...
      '「表示中を一斉削除」で、絞り込み中のメッセージをまとめて削除できます（削除できるのは管理者・マネージャーのみ）。',
    ],
  },
  {
    title: 'スケジュール',
    steps: [
      ...
      'ヘッダーの「週表示／月表示」で表示方法を切り替えられます。月表示では、1か月分の予定をカレンダー形式でまとめて確認できます。',
  ...
  },
  {
    title: '売上管理',
    steps: [
      '「売上台帳」に、確定・見込みの売上を月額契約・案件・手入力の合計で一覧できます。手入力の行は「＋売上を追加」から登録・編集できます。',
      '手入力行は、編集フォームの「顧客管理との紐付け」で顧客管理に登録済みの顧客と紐付けられます。紐付けると顧客名がリンクになり、クリックでその顧客の詳細ページに移動できます（任意項目のため、紐付けなくても登録できます）。',
      '「月次実績・売上予測」タブでは、月別の縦棒グラフと、月ごとのカテゴリ別テーブルの両方で確認できます。グラフには目標を月割りにした点線が入っており、実績が目標ペースに対してどのくらいかひと目でわかります（棒にマウスを乗せると金額を表示）。',
      '「月次実績・売上予測」タブには「成功報酬（採択）入金見込み」の一覧もあります。採択が決まった補助金案件の成功報酬の合計と、案件ごとの金額・採択日を確認できます（実際の入金はクライアントへの補助金支給後になるため、資金繰り確認用です）。',
      '「月額契約」「目標設定」タブで、契約の管理と年間目標・カテゴリ別目標の設定ができます。',
    ],
  },
  {
    title: '請求書発行',
    steps: [
      '請求書・見積書を作成し、画面上でプレビュー・印刷／PDF化・コピーができます。上部のタブで「請求書」「見積書」を切り替えます。',
      '「＋請求書を作成（見積書を作成）」から、請求先・発行日・明細・税率・備考を入力します。顧客登録情報から選ぶと、請求先欄に会社名を自動で入力できます。',
      '明細行は▲▼ボタンで順番を入れ替えられます。行の追加・削除もその場で行えます。',
      '印刷・PDF・プレビューでは、実際に入力した明細行数だけが表示されます（空欄の行は出ません）。「作業内容」欄は、どの明細にも入力がない場合は自動で列ごと省略されます。',
      '一覧の行をクリックすると詳細が開き、そこから編集・コピー・プレビュー・印刷／PDF化ができます。削除は管理者・マネージャーのみ可能です。',
    ],
  },
  {
    title: '実績・売上目標',
