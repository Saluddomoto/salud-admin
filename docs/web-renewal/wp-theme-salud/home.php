...
/**
 * ブログ一覧（投稿ページ）
 * 「設定 > 表示設定 > 投稿ページ」に指定した固定ページで表示されます。
 * @package Salud
 */
get_header();
?>
...
<section class="article-head">
  <div class="wrap">
        ...
        <?php while ( have_posts() ) : the_post(); ?>
          <a class="blog-card" href="<?php the_permalink(); ?>">
            <?php if ( has_post_thumbnail() ) : ?>
              <?php the_post_thumbnail( 'medium_large', array( 'class' => 'thumb' ) ); ?>
            <?php else : ?>
    ...
    <?php else : ?>
      <p class="lede">まだ記事がありません。WordPress管理画面の「投稿 → 新規追加」から書くと、ここと TOP に自動で表示されます。</p>
    <?php endif; ?>
  </div>
</section>
</div>
