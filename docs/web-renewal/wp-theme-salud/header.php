<?php
/**
 * ヘッダー: <head> と サイトヘッダーナビ
 * @package Salud
 */
?><!DOCTYPE html>
<html <?php language_attributes(); ?>>
<head>
  <meta charset="<?php bloginfo( 'charset' ); ?>">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <link rel="profile" href="https://gmpg.org/xfn/11">
  <?php wp_head(); ?>
<style>
.salud-has-dropdown{position:relative}
.salud-dropdown{display:none;position:absolute;top:100%;left:0;min-width:300px;background:#fff;border:1px solid #e2e9ee;border-radius:10px;box-shadow:0 12px 28px rgba(0,0,0,.12);padding:10px;margin:0;padding-top:16px;list-style:none;z-index:100}
.salud-has-dropdown:hover .salud-dropdown,.salud-has-dropdown:focus-within .salud-dropdown{display:block}
.salud-dropdown li{margin:0}
.salud-dropdown a{display:block;padding:13px 16px;font-size:14px;color:var(--ink,#353333);text-decoration:none;border-radius:6px;white-space:normal;line-height:1.5;cursor:pointer}
.salud-dropdown a:hover{background:var(--tint,#f0f8fd);color:var(--teal,#1e9de3)}

.salud-nav-toggle-input{display:none}
.salud-hamburger{display:none;flex-direction:column;justify-content:center;gap:5px;width:32px;height:32px;cursor:pointer;z-index:200;position:relative}
.salud-hamburger span{display:block;width:100%;height:3px;background:var(--ink,#353333);border-radius:2px}
@media (max-width:900px){
.nav-cta{display:none!important}
#hd .logo{display:flex!important;position:fixed!important;top:0;left:16px;height:72px;align-items:center;z-index:999997}
#hd .logo img{height:44px!important;width:auto!important;display:block!important}
#hd .logo .lg-txt{font-size:16.5px!important}
header{height:72px!important;background:#fff!important}
.nav{height:72px!important}
.salud-hamburger{display:flex!important;position:fixed!important;top:20px;right:16px;background:#fff;border:1px solid #e2e9ee;border-radius:8px;padding:8px;box-shadow:0 2px 10px rgba(0,0,0,.15);z-index:999999}
.nav-links{display:none!important;position:fixed!important;top:0!important;left:0!important;right:0!important;width:100%!important;height:100vh!important;height:100dvh!important;background:#fff;flex-direction:column!important;align-items:flex-start!important;gap:0!important;margin:0!important;padding:80px 24px 24px!important;overflow-y:auto;z-index:999998;box-sizing:border-box}
.nav-links li{width:100%;border-bottom:1px solid #e2e9ee}
.nav-links a{display:block;padding:16px 0;font-size:16px;color:#1f2937!important}
.salud-has-dropdown .salud-dropdown{display:none!important;position:static!important;box-shadow:none!important;border:none!important;padding:0 0 8px 16px!important;margin:0!important;min-width:0!important}
.salud-has-dropdown.salud-sub-open .salud-dropdown{display:block!important}
.salud-has-dropdown>a{display:flex!important;align-items:center;justify-content:space-between}
.salud-has-dropdown>a:after{content:"\25BE";font-size:12px;margin-left:8px;transition:transform .2s}
.salud-has-dropdown.salud-sub-open>a:after{transform:rotate(180deg)}
.salud-dropdown a{padding:10px 0!important;font-size:14.5px!important}
.salud-nav-toggle-input:checked ~ .nav-links{display:flex!important}
.nav-links.salud-nav-open{display:flex!important}
}</style>
</head>
<body <?php body_class(); ?>>
<?php wp_body_open(); ?>

<header id="hd">
  <div class="wrap nav">
    <a class="logo" href="<?php echo esc_url( home_url( '/' ) ); ?>"><img src="data:image/png;base64," alt="株式会社Salud"><span class="lg-txt">株式会社Salud</span></a>
    <input type="checkbox" id="salud-nav-toggle" class="salud-nav-toggle-input">
    <label for="salud-nav-toggle" class="salud-hamburger" aria-label="メニューを開く">
      <span></span><span></span><span></span>
    </label>
    <ul class="nav-links">
      <li class="salud-has-dropdown"><a href="<?php echo esc_url( home_url( '/' ) ); ?>#services">サービス</a>
<ul class="salud-dropdown">
<li><a href="<?php echo esc_url( home_url( '/shorikika/' ) ); ?>">中小企業省力化投資補助金</a></li>
<li><a href="<?php echo esc_url( home_url( '/jizokuka/' ) ); ?>">小規模事業者持続化補助金</a></li>
<li><a href="<?php echo esc_url( home_url( '/shinjigyo/' ) ); ?>">新事業進出・ものづくり商業サービス補助金</a></li>
<li><a href="<?php echo esc_url( home_url( '/seicho/' ) ); ?>">成長加速化補助金</a></li>
<li><a href="<?php echo esc_url( home_url( '/shokei/' ) ); ?>">事業承継・M&amp;A補助金</a></li>
<li><a href="<?php echo esc_url( home_url( '/yuushi/' ) ); ?>">融資支援・資金調達サポート</a></li>
<li><a href="<?php echo esc_url( home_url( '/nenkan/' ) ); ?>">年間補助金戦略サポート</a></li>
<a href="<?php echo esc_url( home_url( '/contact/' ) ); ?>" class="salud-mobile-cta">無料相談はこちら →</a>
    </ul>
<script>
(function(){
  var label = document.querySelector('.salud-hamburger');
  var links = document.querySelector('.nav-links');
  var cb = document.getElementById('salud-nav-toggle');
  if(!label || !links) return;

  function closeMenu(){
    links.classList.remove('salud-nav-open');
    if(cb){ cb.checked = false; }
  }

  label.addEventListener('click', function(ev){
    ev.preventDefault();
    var isOpen = links.classList.toggle('salud-nav-open');
    if(cb){ cb.checked = isOpen; }
  });
  label.addEventListener('touchend', function(ev){
    ev.preventDefault();
    var isOpen = links.classList.toggle('salud-nav-open');
    if(cb){ cb.checked = isOpen; }
  });

  var svcParent = document.querySelector('.salud-has-dropdown');
  var svcTrigger = svcParent ? svcParent.querySelector('a') : null;
  if(svcParent && svcTrigger){
    svcTrigger.addEventListener('click', function(ev){
      if(window.matchMedia('(max-width:900px)').matches){
        ev.preventDefault();
        svcParent.classList.toggle('salud-sub-open');
      }
    });
  }

  // メニュー項目を選んだら（サービスの開閉トグル自身を除き）メニューを閉じる
  links.addEventListener('click', function(ev){
    var a = ev.target.closest('a');
    if(!a || a === svcTrigger) return;
    closeMenu();
  });
})();
</script>
</li>
      <li><a href="<?php echo esc_url( home_url( '/' ) ); ?>#works">導入事例</a></li>
      <li><a href="<?php echo esc_url( home_url( '/' ) ); ?>#/support">料金</a></li>
      <li><a href="<?php echo esc_url( home_url( '/' ) ); ?>#/seminar">セミナー</a></li>
      <li><a href="<?php echo esc_url( home_url( '/' ) ); ?>#blog">ブログ</a></li>
      <li><a href="<?php echo esc_url( home_url( '/' ) ); ?>#/faq">よくある質問</a></li>
      <li><a href="<?php echo esc_url( home_url( '/' ) ); ?>#/company">会社概要</a></li>
    </ul>
    <div class="nav-cta">
      <a class="nav-login" href="#">ログイン</a>
      <a class="btn btn-yellow" href="<?php echo esc_url( home_url( '/contact/' ) ); ?>">無料相談</a>
    </div>
  </div>
</header>
