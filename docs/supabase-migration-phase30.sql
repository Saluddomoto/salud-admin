-- ================================================================
-- Phase 30 マイグレーション — 契約書テンプレート(ひな形)の編集機能
-- これまでコードに直書きだったテンプレート本文をDBに移し、画面から
-- 編集できるようにする。本文中の【相手先会社名】【相手先住所】
-- 【代表者名】【契約日】がフォーム入力値に置換される。
...
-- 閲覧は全員可（契約書作成フォームで選択するため）、編集はadmin/managerのみ
CREATE POLICY "contract_templates: authenticated read"
  ON public.contract_templates FOR SELECT
  USING (auth.uid() IS NOT NULL);

CREATE POLICY "contract_templates: manager+ write"
...
INSERT INTO public.contract_templates (key, label, description, title, body_template) VALUES (
  'partner_agreement',
  'パートナー契約合意書（代理店紹介契約）',
  '代理店からのお客様紹介・コミッション支払条件を定める契約書',
  'パートナー契約合意書',
$body$【相手先会社名】（以下「御社」といいます。）は、株式会社Salud（以下「弊社」といいます。）に下記に掲げる条件をもって、お客様を紹介し、弊社はこれに承諾します。
...
弊社）東京都渋谷区道玄坂1-10-8渋谷道玄坂東急ビル2F-C
　　　株式会社Salud
　　　代表取締役　堂本 拓央$body$
);
